...
CodeBuffer::CodeBuffer() : buffer(), globalDefs(), num_variables(0) {}
...
string CodeBuffer::freshVar() {
    ...
    variable << "%r" << num_variables;
    ++num_variables;
    string res(variable.str());
}

string CodeBuffer::freshGlobalVar() {
    stringstream variable;
    variable << "@.r" << this->num_variables;
    ++this->num_variables;
    string res(variable.str());
    return res;
}
